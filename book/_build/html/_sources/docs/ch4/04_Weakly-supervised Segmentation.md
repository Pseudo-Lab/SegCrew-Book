...
- Pixel/instance level의 label이 확보되지 않는 weakly supervised setting에서는 Class attention map (CAM)을 주로 활용한다. (CAM 설명 추가)
...
- 이러한 CAM 정보는 class label만이 주어진 환경에서 이미지 내에 객체가 어디에 속한지 알려주지만 다음과 같은 문제점이 있다.
    
    (1) CAM의 결과가 객체의 Discriminative한 영역에만 한정됨 
    
    (2) CAM의 크기가 객체의 영역외에 나타나는 모습을 보임 
